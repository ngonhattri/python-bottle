...
def add():
    ...
    if request.method == "GET" or request.forms.get('next') == 'back':
        return form_html.replace('<!--user_id-->', '').replace('<!--passwd-->', '').replace('<!--email-->', '').replace('<!--user_shi-->', '').replace('<!--user_mei-->', '')
    else:
        #postされたフォームの値を取得する
        form = {}
        ...
        #受け取った値を置換する
        #メソッドは重ね掛けできる
        return html.replace('<!--user_id-->', form['user_id']).replace('<!--passwd-->', form['passwd']).replace('<!--email-->', form['email']).replace('<!--user_shi-->', form['user_shi']).replace('<!--user_mei-->', form['user_mei'])
...
@app.route('/list')
def list():
    sql = """select user_id, email, user_shi, user_mei from book_user where del = false order by user_id asc """
